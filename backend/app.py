    ...
    try:
        ...
        if is_gagansaathi or (from_airport and to_airport):
            prompt = (
                ...
                f"Deliver a warm, highly detailed, beautifully structured response with the following sections in rich narrative paragraphs:\n\n"
                ...
                f"1. ✈️ **Trip Overview & Flight Logistics (End-to-End Roadmap)**:\n"
                f"   • **Flight Distance & Direct Cruising Time**: State the estimated distance and ~1h 30m flight duration.\n"
                f"   • **Pre-Departure Instructions**: When to start from home with traffic buffer, 24h web check-in, keeping ID & digital boarding pass ready, and packing powerbanks strictly in cabin baggage.\n"
                f"   • **At Origin Airport**: Terminal entry gate ID verification, baggage drop counter protocol, CISF security trays (electronics separate), and boarding gate timing.\n"
                f"   • **On Arrival at Destination Airport**: Step-by-step guidance on baggage claim carousel collection, and choosing between **Destination Metro Line vs Official Prepaid Airport Taxi Kiosks vs App-Based Cabs (Uber/Ola)**.\n\n"
                f"2. 🌤️ **3-Day Weather Prediction & Packing Gear Advisory**:\n"
                f"   • Provide a day-by-day weather forecast for **{trip_days}** (temperatures, rain probability, sky conditions).\n"
                f"   • Recommend preventive gear (e.g. ☔ Umbrella / Raincoat if rain is likely, 🕶️ Sunscreen/Sunglasses if sunny/UV high, 🧥 Light jackets, comfortable footwear).\n"
                f"   • Explain weather-adaptive activity ideas (e.g. indoor museums for rainy hours, early morning/sunset shaded visits for hot sunny hours, outdoor lakes for pleasant weather).\n\n"
                f"3. 🏥 **Nearest 24/7 Emergency Hospitals & Helplines**:\n"
                f"   • Upfront Emergency Helplines: Police (`100`/`112`), Ambulance (`108`), Women Safety Helpline (`181`).\n"
                f"   • Recommend 3 top multi-specialty emergency hospitals near the airport / city center with 24/7 helplines, distance, and trauma care facilities.\n\n"
                f"4. 🏨 **Nearest Hotel & Living Accommodations**:\n"
                f"   • Suggest 3 top-rated hotels matching the budget tier ({budget or 'Moderate'}).\n"
                f"   • For each hotel, give the Hotel Name, approximate distance from airport, key amenities, estimated nightly tariff, weather suitability, and brief description.\n\n"
                f"5. 🍽️ **Food, Cuisine & Allergy Advisory**:\n"
                f"   • Tailored strictly to **{food_pref or 'all dietary preferences'}** and respecting allergies: **{allergies or 'general safety'}**.\n"
                f"   • List 3 recommended foodlets / restaurants / food centers near the airport & downtown, highlight iconic safe dishes to try, and give clear allergen precautions for the kitchen.\n\n"
                f"6. 🛡️ **Night Safety & Crime Alert Advisory**:\n"
                f"   • Provide an honest destination safety advisory around the airport and city after dark.\n"
                f"   • Essential DOs and critical DON'Ts, verified transit recommendations, and areas to avoid late at night.\n\n"
                f"7. 📍 **Famous Viewpoints & City Attractions**:\n"
                f"   • List top 4 iconic viewpoints, cultural landmarks, and sightseeing attractions with the best visiting times and weather setting.\n\n"
